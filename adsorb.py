...
def add_adsorbate_by_coordinate(
    atoms: Atoms, adsorbate: Atoms | str, coords: list[float]
) -> Atoms:
...
def add_adsorbate_by_index(atoms: Atoms, adsorbate: Atoms | str, index: int) -> Atoms:
    ...
    atoms.extend(
        Atoms(
            adsorbate, positions=[atoms[index].position[0], atoms[index].position[1], z]
        )
    )
...
def get_surface_atoms(atoms: Atoms, tolerance: float = 0.01) -> dict:
    ...
    surface_atoms: dict = {
        ...
        "types": surface_atom_types,
        "coordinates": surface_atom_coordinates,
    }
...
def print_surface_atoms(surface_atoms_dictionary: dict):
    ...
    for atom in surface_atoms_dictionary["indices"]:
        print(
            f"{surface_atoms_dictionary['types'][atom]} {atom} {surface_atoms_dictionary['coordinates'][atom]}"
        )
...
def adsorbate_utilities():
    parser = argparse.ArgumentParser(
        description="A utility for adsorbing molecules on surfaces"
    )
    ...
    parser.add_argument(
        ...
        "--file",
        type=str,
        help="The structure file to be adsorbed",
        required=True,
    )
    parser.add_argument(
        "-a",
        "--adsorbate",
        type=str,
        help="The adsorbate to be adsorbed in string format",
        required=False,
    )
    parser.add_argument(
        "-A",
        "--adsorbate-file",
        type=str,
        help="Allows the user to specify a file containing the adsorbate to be adsorbed",
    )
    parser.add_argument(
        "-c",
        "--coordinates",
        type=float,
        nargs=3,
        help="The coordinates of the adsorbate",
        required=False,
    )
    # adsorbate index -i or --index
    parser.add_argument(
        "-I", "--index", type=int, help="The index of a target atom", required=False
    )
    # output file -o or --output, default is to print to the contents of the file to stdout
    parser.add_argument(
        "-o", "--output", type=str, help="Name of the output file", required=False
    )
    parser.add_argument(
        "--pbc",
        type=bool,
        nargs=3,
        help="Sets the pbc of the structure file",
        required=False,
    ...
    parser.add_argument(
        "--surface",
        action="store_true",
        help="Prints the surface atoms of the structure file",
        required=False,
    )
