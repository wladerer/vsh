...
def kpoints_coordinates_from_vasprun(vasprun: Vasprun) -> list[list[float,float,float]]:
    '''Gets the kpoints from a vasprun file'''
    kpoints: list[list[float,float,float]] = vasprun.actual_kpoints
    return kpoints

def get_fermi_band_indices(dataframe: pd.DataFrame, efermi: float, tolernace: float = 0.1) -> list[int]:
...
def update_dataframe_kpoint_coordinates(dataframe: pd.DataFrame, kpoint_coordinates: list[list[float,float,float]]) -> pd.DataFrame:
...
def get_fermi_surface_kpoint_indices(dataframe: pd.DataFrame, bands: list[int]) -> list[int]:
...
kpoint_coords = kpoints_coordinates_from_vasprun(Vasprun('/Users/wladerer/Downloads/bi2te3/vasprun.xml', parse_dos=False, parse_eigen=False, parse_projected_eigen=False, parse_potcar_file=False))
