...
def write_kpoints(args):
    ...
    if args.mesh_type == "monkhorst":
        ...
        if not args.input:
            raise ValueError(
                "No input structure file provided. Automatic Density requires structure"
            )
...
def append_hybrid_mesh(args):
    ...
    with open(args.input, "r") as f:
        ...
        grid_string = hybrid_mesh_to_string(
            hydbrid_mesh(step=args.step, weight=args.weight)
        )
...
def run(args):
    functions = {"mesh": write_kpoints, "path": write_path, "plane": write_plane}
