...
def plot_linear_slice(file: str, point1: list[float, float, float], point2: list[float, float, float], npoints: int = 100):
    '''Plots a linear slice of the CHGCAR file. '''
    from matplotlib import pyplot as plt
    import numpy as np

    chgcar = Chgcar.from_file(file)
    charge_line = chgcar.linear_slice(point1, point2, npoints) # 1d array of values along the slice
