...
def cohp_from_dir(directory: str):
    ...
    cohp = CompleteCohp.from_file(
        fmt="LOBSTER",
        filename=directory + "/COHPCAR.lobster",
        structure_file=directory + "/POSCAR",
    )
...
def check_cohp_path(input_path):
    ...
    if os.path.isfile(input_path):
        ...
        raise ValueError(
            "Invalid input. Please provide a valid file path or directory."
        )
...
def collate_cohps(args):
    ...
    for cohp_object in cohp_list:
        plotter.add_cohp(
            label=args.label, cohp=cohp_object.get_cohp_by_label(str(args.label))
        )
...
def plot_icohp_graph(args):
    ...
    for G in graphs:
        ...
        nx.draw_networkx_labels(
            G, pos, font_size=12, font_family="sans-serif", font_color="white"
        )
        plt.axis("off")
